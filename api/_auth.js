...
function hacherMotDePasse(motDePasse) {
  ...
  const hash = crypto.pbkdf2Sync(motDePasse, sel, 100000, 64, "sha512").toString("hex");
  return `${sel}:${hash}`;
}

function verifierMotDePasse(motDePasse, motDePasseHache) {
  const [sel, hash] = motDePasseHache.split(":");
  const hashTest = crypto.pbkdf2Sync(motDePasse, sel, 100000, 64, "sha512").toString("hex");
...
function verifierJeton(jeton, secret) {
  try {
    ...
    const signatureAttendue = crypto.createHmac("sha256", secret).update(payload).digest("base64url");
    if (!crypto.timingSafeEqual(Buffer.from(signature), Buffer.from(signatureAttendue))) {
      return null;
    }
